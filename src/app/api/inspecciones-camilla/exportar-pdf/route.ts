...
interface DetalleCamilla {
  id: string;
  camillaNombre: string;
...
// ══════════════════════════════════════════════════════════
// Generar PDF de inspección de camilla
// ══════════════════════════════════════════════════════════
function generarPDFInspeccion(insp: InspeccionCompleta, logoBase64: string | null): Buffer {
  ...
  doc.setFont("helvetica", "bold");
  doc.setFontSize(7);
  doc.text("FORMATO INSPECCIÓN DE CAMILLAS", empresaX + empresaWidth / 2, currentY + 2 * headerHeight / 3 + 5, { align: "center" });

  const codigoX = margin + logoWidth + empresaWidth;
  doc.rect(codigoX, currentY, codigoWidth, headerHeight / 3, "S");
  doc.setFontSize(8);
  doc.text("CÓDIGO: FT-SST-068", codigoX + codigoWidth / 2, currentY + 5, { align: "center" });
  ...
  if (insp.detalles.length === 0) {
    ...
    // Agrupar por camilla
    const detallesPorCamilla: Record<string, DetalleCamilla[]> = {};
    insp.detalles.forEach((det) => {
      const camilla = det.camillaNombre || "Sin Camilla";
      if (!detallesPorCamilla[camilla]) detallesPorCamilla[camilla] = [];
      detallesPorCamilla[camilla].push(det);
    });

    const camillas = Object.keys(detallesPorCamilla).sort();

    camillas.forEach((camilla) => {
      const elementos = detallesPorCamilla[camilla];
      ...
      // Título de la camilla
      doc.setFillColor(46, 125, 50);
      doc.rect(margin, currentY, contentWidth, 7, "FD");
      doc.setFontSize(9);
      doc.setFont("helvetica", "bold");
      doc.setTextColor(...COLORS.WHITE);
      doc.text(`${camilla.toUpperCase()} (${elementos.length} elementos)`, margin + contentWidth / 2, currentY + 5, { align: "center" });
      doc.setTextColor(...COLORS.BLACK);
      currentY += 7;
...
// ══════════════════════════════════════════════════════════
// POST /api/inspecciones-camilla/exportar-pdf
// ══════════════════════════════════════════════════════════
export async function POST(request: NextRequest) {
  try {
    ...
    if (idInspeccion) {
      extraParams.filterByFormula = `{${inspCamillaFields.ID}} = '${idInspeccion}'`;
    }
    ...
    const detalleFilter = `FIND('${inspRecord.id}', {${detalleCamillaFields.INSPECCION_LINK}}) > 0`;
    const detalleRecords = await fetchAllRecords(detalleUrl, getSGSSTHeaders(), { filterByFormula: detalleFilter });

    // ── 3. Obtener nombres de camillas ──────────────────
    const camillaIds = detalleRecords
      .map((r) => (r.fields[detalleCamillaFields.CAMILLA_LINK] as string[])?.[0])
      .filter(Boolean) as string[];

    const camillaNames: Record<string, string> = {};
    const uniqueCamillaIds = [...new Set(camillaIds)];

    for (let i = 0; i < uniqueCamillaIds.length; i += 10) {
      const batch = uniqueCamillaIds.slice(i, i + 10);
      const formula = `OR(${batch.map((cid) => `RECORD_ID()='${cid}'`).join(",")})`;
      ...
      const cmRes = await fetch(
        `${getSGSSTUrl(airtableSGSSTConfig.camillasTableId)}?${cmParams.toString()}`,
        { headers: getSGSSTHeaders(), cache: "no-store" }
      );

      if (cmRes.ok) {
        const cmData: AirtableResponse = await cmRes.json();
        cmData.records.forEach((r) => {
          camillaNames[r.id] = (r.fields[camillasFields.NOMBRE] as string) || (r.fields[camillasFields.CODIGO] as string) || "";
        });
      }
    }
    ...
    const elementoIds = detalleRecords
      .map((r) => (r.fields[detalleCamillaFields.ELEMENTO_LINK] as string[])?.[0])
      .filter(Boolean) as string[];
    ...
    for (let i = 0; i < uniqueElementoIds.length; i += 10) {
      ...
      const elRes = await fetch(
        `${getSGSSTUrl(airtableSGSSTConfig.elementosCamillaTableId)}?${elParams.toString()}`,
        { headers: getSGSSTHeaders(), cache: "no-store" }
      );
      ...
      if (elRes.ok) {
        ...
        elData.records.forEach((r) => {
          elementoNames[r.id] = (r.fields[elementosCamillaFields.NOMBRE] as string) || "";
        });
      }
    }

    // ── 5. Mapear detalles ──────────────────────────────
    const detalles: DetalleCamilla[] = detalleRecords.map((r) => {
      const camillaLink = (r.fields[detalleCamillaFields.CAMILLA_LINK] as string[])?.[0];
      const elementoLink = (r.fields[detalleCamillaFields.ELEMENTO_LINK] as string[])?.[0];

      return {
        id: r.id,
        camillaNombre: camillaLink ? camillaNames[camillaLink] : "",
        elementoNombre: elementoLink ? elementoNames[elementoLink] : "",
        estadoElemento: (r.fields[detalleCamillaFields.ESTADO_ELEMENTO] as string) || "",
        cantidad: (r.fields[detalleCamillaFields.CANTIDAD] as string) || "",
        observaciones: (r.fields[detalleCamillaFields.OBSERVACIONES] as string) || "",
      };
    });

    // ── 6. Obtener responsables ─────────────────────────
    const respUrl = getSGSSTUrl(airtableSGSSTConfig.respCamillaTableId);
    const respFilter = `FIND('${inspRecord.id}', {${respCamillaFields.INSPECCION_LINK}}) > 0`;
    ...
    const responsables: Responsable[] = respRecords.map((r) => ({
      id: r.id,
      tipo: (r.fields[respCamillaFields.TIPO] as string) || "",
      nombre: (r.fields[respCamillaFields.NOMBRE] as string) || "",
      cargo: (r.fields[respCamillaFields.CARGO] as string) || "",
      firmado: (r.fields[respCamillaFields.FIRMADO] as boolean) || false,
      fechaFirma: (r.fields[respCamillaFields.FECHA_FIRMA] as string) || "",
    }));

    // ── 7. Construir inspección completa ────────────────
    const inspeccion: InspeccionCompleta = {
      recordId: inspRecord.id,
      idInspeccion: (inspRecord.fields[inspCamillaFields.ID] as string) || "",
      fecha: (inspRecord.fields[inspCamillaFields.FECHA] as string) || "",
      inspector: (inspRecord.fields[inspCamillaFields.INSPECTOR] as string) || "",
      cargoInspector: (inspRecord.fields[inspCamillaFields.CARGO_INSPECTOR] as string) || "",
      estado: (inspRecord.fields[inspCamillaFields.ESTADO] as string) || "",
      observaciones: (inspRecord.fields[inspCamillaFields.OBSERVACIONES] as string) || "",
      detalles,
      responsables,
    };
    ...
    const filename = `Inspeccion_Camilla_${inspeccion.idInspeccion}.pdf`;
    ...
    console.error("Error en POST /api/inspecciones-camilla/exportar-pdf:", error);
    return NextResponse.json(
