...
interface DetalleBotiquin {
  id: string;
  botiquinNombre: string;
...
// ══════════════════════════════════════════════════════════
// Generar PDF de inspección de botiquín
// ══════════════════════════════════════════════════════════
function generarPDFInspeccion(insp: InspeccionCompleta, logoBase64: string | null): Buffer {
  ...
  doc.setFont("helvetica", "bold");
  doc.setFontSize(7);
  doc.text("FORMATO INSPECCIÓN DE BOTIQUINES", empresaX + empresaWidth / 2, currentY + 2 * headerHeight / 3 + 5, { align: "center" });

  const codigoX = margin + logoWidth + empresaWidth;
  doc.rect(codigoX, currentY, codigoWidth, headerHeight / 3, "S");
  doc.setFontSize(8);
  doc.text("CÓDIGO: FT-SST-067", codigoX + codigoWidth / 2, currentY + 5, { align: "center" });
  ...
  // ═══════════════════════════════════════════════════════
  // TABLA DE DETALLES POR BOTIQUÍN
  // ═══════════════════════════════════════════════════════
  if (insp.detalles.length === 0) {
    ...
    // Agrupar por botiquín
    const detallesPorBotiquin: Record<string, DetalleBotiquin[]> = {};
    insp.detalles.forEach((det) => {
      const botiquin = det.botiquinNombre || "Sin Botiquín";
      if (!detallesPorBotiquin[botiquin]) detallesPorBotiquin[botiquin] = [];
      detallesPorBotiquin[botiquin].push(det);
    });

    const botiquines = Object.keys(detallesPorBotiquin).sort();

    botiquines.forEach((botiquin) => {
      const elementos = detallesPorBotiquin[botiquin];
      ...
      doc.text(`${botiquin.toUpperCase()} (${elementos.length} elementos)`, margin + contentWidth / 2, currentY + 5, { align: "center" });
      doc.setTextColor(...COLORS.BLACK);
      currentY += 7;
...
// ══════════════════════════════════════════════════════════
// POST /api/inspecciones-botiquin/exportar-pdf
// ══════════════════════════════════════════════════════════
export async function POST(request: NextRequest) {
  try {
    ...
    if (idInspeccion) {
      extraParams.filterByFormula = `{${inspBotiquinFields.ID}} = '${idInspeccion}'`;
    }
    ...
    const detalleFilter = `FIND('${inspRecord.id}', {${detalleBotiquinFields.INSPECCION_LINK}}) > 0`;
    const detalleRecords = await fetchAllRecords(detalleUrl, getSGSSTHeaders(), { filterByFormula: detalleFilter });

    // ── 3. Obtener nombres de botiquines ────────────────
    const botiquinIds = detalleRecords
      .map((r) => (r.fields[detalleBotiquinFields.BOTIQUIN_LINK] as string[])?.[0])
      .filter(Boolean) as string[];

    const botiquinNames: Record<string, string> = {};
    const uniqueBotiquinIds = [...new Set(botiquinIds)];

    for (let i = 0; i < uniqueBotiquinIds.length; i += 10) {
      const batch = uniqueBotiquinIds.slice(i, i + 10);
      const formula = `OR(${batch.map((bid) => `RECORD_ID()='${bid}'`).join(",")})`;
      const bqParams = new URLSearchParams({
        filterByFormula: formula,
        returnFieldsByFieldId: "true",
      });

      const bqRes = await fetch(
        `${getSGSSTUrl(airtableSGSSTConfig.botiquinesTableId)}?${bqParams.toString()}`,
        { headers: getSGSSTHeaders(), cache: "no-store" }
      );

      if (bqRes.ok) {
        const bqData: AirtableResponse = await bqRes.json();
        bqData.records.forEach((r) => {
          botiquinNames[r.id] = (r.fields[botiquinesFields.NOMBRE] as string) || (r.fields[botiquinesFields.CODIGO] as string) || "";
        });
      }
    }
    ...
    const elementoIds = detalleRecords
      .map((r) => (r.fields[detalleBotiquinFields.ELEMENTO_LINK] as string[])?.[0])
      .filter(Boolean) as string[];
    ...
    for (let i = 0; i < uniqueElementoIds.length; i += 10) {
      ...
      const elRes = await fetch(
        `${getSGSSTUrl(airtableSGSSTConfig.elementosBotiquinTableId)}?${elParams.toString()}`,
        { headers: getSGSSTHeaders(), cache: "no-store" }
      );
      ...
      if (elRes.ok) {
        ...
        elData.records.forEach((r) => {
          elementoNames[r.id] = (r.fields[elementosBotiquinFields.NOMBRE] as string) || "";
        });
      }
    }

    // ── 5. Mapear detalles ──────────────────────────────
    const detalles: DetalleBotiquin[] = detalleRecords.map((r) => {
      const botiquinLink = (r.fields[detalleBotiquinFields.BOTIQUIN_LINK] as string[])?.[0];
      const elementoLink = (r.fields[detalleBotiquinFields.ELEMENTO_LINK] as string[])?.[0];

      return {
        id: r.id,
        botiquinNombre: botiquinLink ? botiquinNames[botiquinLink] : "",
        elementoNombre: elementoLink ? elementoNames[elementoLink] : "",
        estadoElemento: (r.fields[detalleBotiquinFields.ESTADO_ELEMENTO] as string) || "",
        cantidad: (r.fields[detalleBotiquinFields.CANTIDAD] as string) || "",
        fechaVencimiento: (r.fields[detalleBotiquinFields.FECHA_VENCIMIENTO] as string) || "",
        observaciones: (r.fields[detalleBotiquinFields.OBSERVACIONES] as string) || "",
      };
    });

    // ── 6. Obtener responsables ─────────────────────────
    const respUrl = getSGSSTUrl(airtableSGSSTConfig.respBotiquinTableId);
    const respFilter = `FIND('${inspRecord.id}', {${respBotiquinFields.INSPECCION_LINK}}) > 0`;
    ...
    const responsables: Responsable[] = respRecords.map((r) => ({
      id: r.id,
      tipo: (r.fields[respBotiquinFields.TIPO] as string) || "",
      nombre: (r.fields[respBotiquinFields.NOMBRE] as string) || "",
      cargo: (r.fields[respBotiquinFields.CARGO] as string) || "",
      firmado: (r.fields[respBotiquinFields.FIRMADO] as boolean) || false,
      fechaFirma: (r.fields[respBotiquinFields.FECHA_FIRMA] as string) || "",
    }));

    // ── 7. Construir inspección completa ────────────────
    const inspeccion: InspeccionCompleta = {
      recordId: inspRecord.id,
      idInspeccion: (inspRecord.fields[inspBotiquinFields.ID] as string) || "",
      fecha: (inspRecord.fields[inspBotiquinFields.FECHA] as string) || "",
      inspector: (inspRecord.fields[inspBotiquinFields.INSPECTOR] as string) || "",
      cargoInspector: (inspRecord.fields[inspBotiquinFields.CARGO_INSPECTOR] as string) || "",
      estado: (inspRecord.fields[inspBotiquinFields.ESTADO] as string) || "",
      observaciones: (inspRecord.fields[inspBotiquinFields.OBSERVACIONES] as string) || "",
      detalles,
      responsables,
    };
    ...
    const filename = `Inspeccion_Botiquin_${inspeccion.idInspeccion}.pdf`;
    ...
    console.error("Error en POST /api/inspecciones-botiquin/exportar-pdf:", error);
    return NextResponse.json(
