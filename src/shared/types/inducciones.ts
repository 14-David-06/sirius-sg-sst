...
export const RegistroInduccionSchema = z.object({
  ...
  idEmpleadoCore: z.string().regex(/^SIRIUS-PER-\d{4}$/), // ID Empleado (SIRIUS-PER-XXXX)
  ...
  firmaResponsableSST: z.string().optional().nullable(), // Firma cifrada del responsable SST
  certificadoUrl: z.string().url().optional().nullable(),
  documentoSnapshotUrl: z.string().url().optional().nullable(), // Snapshot JSON en S3 para regeneración
